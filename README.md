...
**Project Aurora automates the entire chain.** Given only a company knowledge base and a user-activity CSV, it produces a complete notification strategy (segments, goals, themes, bilingual copy, timing, and a full delivery schedule), then, given real campaign results, autonomously evolves that strategy for the next iteration, with a full audit trail of what changed and why.
...
- **Domain-agnostic by construction.** Swap the knowledge base and the CSV for a different company (fintech, SaaS, e-commerce) and the entire pipeline re-targets itself with no code changes. Features, tones, goals, and segments are all discovered at runtime, never hardcoded.
...
### How a user gets segmented

Segmentation (`segmentation_engine.py`) is fully deterministic: no LLM call is involved.

```mermaid
flowchart LR
    A["Raw user row"] --> B["Compute activeness_score<br/>0.30·Recency + 0.40·Frequency + 0.30·Magnitude"]
    A --> C["Compute propensity_&lt;feature&gt;<br/>per discovered feature_* column"]
    C --> D["dominant_propensity<br/>= highest propensity feature"]
    B --> E["activeness_band<br/>low / moderate / high<br/>(33rd/67th percentile cut)"]
    A --> F["lifecycle_stage<br/>trial / paid / churned / inactive"]
    E --> G{"Segment assignment"}
    D --> G
    F --> G
    G --> H["SEG_01 … SEG_14<br/>written to user_segments.csv"]
```

### The self-learning loop (Task 3 internals)

`learning_engine.py` runs five phases in sequence, each consuming the last:

```mermaid
flowchart LR
    P1["1. Ingest & Classify<br/>load experiment_results.csv<br/>tag GOOD / NEUTRAL / BAD<br/>compute uninstall guardrails"] --> P2["2. Resolve Timing<br/>shift each segment to its<br/>best-observed window"]
    P2 --> P3["3. Evolve Templates<br/>GOOD kept · NEUTRAL LLM-sharpened<br/>BAD LLM-rewritten + new theme"]
    P3 --> P4["4. Regenerate Schedule<br/>re-map new templates/windows<br/>apply frequency-cut guardrails"]
    P4 --> P5["5. Delta Report<br/>write causal audit row<br/>per change"]
```
...
| `performance_status` | n/a | `BAD` (ctr 0.037, engagement 0.113, uninstall 0.028) |
...
| `source_template_id` | n/a | `TPL_SEG_06_ACTIVATION_..._01` (A/B lineage preserved) |
...
4. **`python main.py --steps task3`** runs the learning engine:
   ...
   - Keeps GOOD templates, sharpens NEUTRAL ones, and fully rewrites BAD ones (e.g. `TPL_SEG_06_ACTIVATION_..._01` becomes `..._01_v2`, new theme, new copy, lineage preserved via `source_template_id`).
...
- **Robust JSON extraction from a small local model.** `llm.parse_json()` degrades through three tiers: standard parse, then control-character/escape sanitization, then regex field extraction, so a 3B-parameter model's imperfect JSON almost never breaks a downstream step. `safe_parse_json()` guarantees a usable value even on total failure.
- **Prompt decomposition over monolithic prompts.** North-star generation uses a 3-layer approach (explicit extraction, then structured scoring, then JSON assembly); tone/hook generation uses a 2-pass split; feature mapping, goals, themes, and templates all use one small, tightly-scoped LLM call per item rather than one giant prompt. This is measurably more reliable on a 3B model, and failures are isolated per item.
...
- **Concurrency with backpressure.** The two heaviest LLM stages (`comm_themes.py`, `message_template_gen.py`) parallelize Ollama calls via `ThreadPoolExecutor(max_workers=2)` for faster wall-clock time without overwhelming a locally-hosted model.
